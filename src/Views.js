...
function oa_requestBuilder_ (auth, stub, http, {payload={}, query={}}={}, mixin) {
  ...
  return endpoint.createRequest(http, {
    stub
  }, {
    payload,
    query
...
function mb_requestBuilder_(auth, stub, http, {payload={}, query={}}={}) {
  const {token, domain, subdomain, version, count} = auth;
  const endpoint = initWithAuthToken_({token, domain, subdomain, count, version})
  return endpoint.createRequest(http, {
    stub
  },{
    payload,
