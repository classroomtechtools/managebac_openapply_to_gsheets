...
function randomByType_(data) {
  // return variable number of
  if (data == null || data === undefined) return null;
  if (typeof data === "string") {
    return faker.datatype.string(data.length);
  }
...
function protectMBStudent_(student) {
  ...
  makeFake(student, "graduating_year", () =>
    Utilities.formatDate(faker.date.soon(), "UTC", "YYYY")
  );
...
function protectNote_(note) {
  ...
  makeFake(note, "reported_by", () =>
    faker.fake("{{name.firstName}} {{name.lastName}}")
  );
  makeFake(note, "homeroom_advisor", () =>
    faker.fake("{{name.firstName}} {{name.lastName}}")
  );
...
function protectAttendance_(attendance) {}
