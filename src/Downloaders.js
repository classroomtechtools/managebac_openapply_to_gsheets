function downloaders_() {
    return {
        ...
        dl_mb_yeargroups: dl_mb_yeargroups_,
        ...
        dl_mb_class_timetables: dl_mb_class_timetables_,
        dl_oa_main: downloadOAv3_2_,
        dl_mb_classAttendance_bydate: dl_mb_classAttendance_bydate_,
        dl_mb_hrAttendance_bydate: dl_mb_hrAttendance_bydate_
    };
}

function get_termLookup_({ endpoint }) {
...
function dl_mb_hrAttendance_bydate_({ endpoint }, downloaderKeywords = {}) {
    const { start_date, end_date, year_group_ids, weekend = [0, 6] } = downloaderKeywords;
    const batch = Endpoints.batch();

    const from_date = new Date(start_date);
    const to_date = new Date(end_date);

    // loop for every day with nested loop for each class
    for (const day = from_date; day <= to_date; day.setDate(day.getDate() + 1)) {
        if (weekend.includes(day.getDay())) continue;

        const query_date = `${day.getFullYear()}` + '-' +
            `${day.getMonth() + 1}`.padStart(2, "0") + '-' +
            `${day.getDate()}`.padStart(2, "0");
        for (const year_group_id of year_group_ids) {
            const request = endpoint.createRequest('get', {
                stub: `year-groups/${year_group_id}/homeroom/attendance/date/${query_date}`
            }, {}, {
                date: query_date
            });
            batch.add({ request });
    ...
    const getStudent = (id) => {
        ...
        const stu = request.fetch().json.student;
        studentCache.set(id, stu);
        return stu;
    }
    ...
    const getTeacher = (id) => {
        ...
        const teach = request.fetch().json.teacher;
        teacherCache.set(id, teach);
        return teach;
    }

    const records = [];
    for (const response of batch) {
        const date = response.request.date;
        const json = response.json;
        for (const student of json.students || []) {
            const stu = getStudent(student.id);
            const teach = getTeacher(stu.homeroom_advisor_id);
            const record = {
                student: {
                    id: stu.id,
                    student_id: stu.student_id,
                    name: `${stu.first_name} ${stu.last_name}`,
                    grade: stu.class_grade_number
                },
                teacher: {
                    id: teach.id,
            ...
            if (Object.keys(student.attendance).length === 0) {
                ...
                record.status = student.attendance.status;
                record.note = student.attendance.note;
                record.date = date
            }
            records.push(record);
        }
...
function dl_mb_classAttendance_bydate_({ endpoint }, downloaderKeywords = {}) {
    const { start_date, end_date, weekend = [0, 6] } = downloaderKeywords;

    // use memberships endpoint to get list of class ids that occur during this period
    // does two calls to that endpoint, one for start date, the end date, (and if there is any difference
    // there, for the mid-point), that's what is uses
    // FIXME: This works fine for most durations (across three terms) but concievable will not work
    //        for durations such as entire year, when there are more than three terms

    // callback that returns all class_ids encoutered (as a list)
    const memberships_callback = (response) =>
        response.memberships.map(membership => membership.class_id);

    const starting_class_ids = getWithPagination_({ endpoint, callback: memberships_callback, stub: 'memberships' }, {
        class_happens_on: start_date
    });

    const ending_class_ids = getWithPagination_({ endpoint, callback: memberships_callback, stub: 'memberships' }, {
        class_happens_on: start_date
    });

    // Use set math to detect if there is any need to continue processing the mid point
    const class_ids = new Set([...starting_class_ids, ...ending_class_ids]);

    // okay, set up the batch requests for all the calls we need 
    // phew, this is gonna be a lot, good thing we have a library that helps us with that
    const batch = Endpoints.batch();
    ...
    for (const day = from_date; day <= to_date; day.setDate(day.getDate() + 1)) {
        ...
        for (const class_id of class_ids) {
            const request = endpoint.createRequest("get", {
                stub: `classes/${class_id}/attendance/date/${query_date}`
            }, {
                includeArchivedStudents: '1'
            }, {
                date: query_date,
                class_id: class_id
            });
    ...
    const getClass = (id) => {
        ...
        const clss = request.fetch().json;
        classCache.set(id, clss.class);
        return clss;
    }
    const studentCache = new Map();
    const getStudent = (id) => {
    ...
    const records = [];
    for (const response of batch) {
        const { class_id, date } = response.request;
        const json = response.json;
        for (const student of json.students || []) {
            if (Object.keys(student.attendance).length === 0) {
                continue;
            ...
            const stu = getStudent(student.id);
            const clss = getClass(class_id);
            const record = {
                date,
                'class': {
                    id: clss.id,
                    uniq_id: clss.uniq_id
                },
                student: {
                    id: stu.id,
                    student_id: stu.student_id,
            ...
            for (const [key, attendance] of Object.entries(student.attendance)) {
                record.period = key;
...
function dl_mb_attendance_({
    ...
    const { termLookup, termIds } = get_termLookup_({ endpoint });
...
function dl_mb_term_grades_for_a_term_({ term_id = null }) {
    ...
    return ({
        ...
        const classes = get_all_classes_({ endpoint, protectData, activeOnly: false }).filter(klass => klass.program_code === term.programme);
...
function dl_mb_academic_terms_for_date_({ date = null }) {
    ...
    return ({ endpoint }) => {
        const { termLookup } = get_termLookup_({ endpoint });
        //return terms;
        const current_terms = Object.entries(termLookup).filter(([id, term]) => {
            ...
            // the time is between the start time and end time
...
/**
 * There is a bug … archived classes timetables cannot be retrieved
 */
function dl_mb_class_timetables_({ endpoint, activeOnly = true, protectData = false }) {

    // callback that goes through each one
    const callback = ({ classes }) => {
        ...
        for (const clss of classes) {
            const request = endpoint.createRequest(
                "get", {
                stub: `classes/${clss.id}/timetable`
            },
                {},
                {
                    id: clss.id,
        ...
        for (const response of batch) {
            if (!response.ok) throw new Error(`${response.statusCode} ${response.text}`);
            const content = response.json.timetable;
            for (const academic_year of content.academic_years) {
                for (const slot of academic_year.slots) {
                    const { slots: remove, ...academic_year_only } = academic_year;
                    const json = { academic_year: academic_year_only, class_id: response.request.id };
                    for (const [key, value] of Object.entries(slot)) {
                        json[key] = value;
                    }
                    timetables.push(json);
                }
            }
        }
    ...
    const responses = getWithPagination_({
        endpoint,
        callback,
        stub: "classes",
    });
    ...
    if (!activeOnly) {
        Logger.log("Cannot get timetables of archived classes");
        // #FIXME: if this bug is fixed    
        // const archivedClasses = getWithPagination_(
        //   {
        //     endpoint,
        //     callback,
...
function dl_mb_classes_({ endpoint, activeOnly = false, protectData = false }) {
    ...
    const callback = ({ classes }) => {
        ...
        for (const response of batch) {
            ...
            if (protectData) protectMBTeacher_(teacher);
...
function dl_mb_yeargroups_({ endpoint }) {
    const callback = ({ year_groups }) => year_groups;

    return getWithPagination_({ endpoint, stub: 'year-groups', callback });
}
