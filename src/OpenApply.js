...
function openApplyUpdater_ ( {clientId, clientSecret, subdomain, count=30},
                           {id, syncKey},
                           {isIncremental=false, protectData=false,
                            priorityHeaders=['id', 'student_id', 'first_name', 'last_name', 'class_grade', 'email'],
                            includeCustomFields=[],
                            includeParentFields=[],
                            activeOnly=false, statuses=[], tabbedStatuses=false, visibility='PROJECT' }={} ) 
{
  ...
  Enforce.named(arguments, {
    ...
    statuses: 'array', isIncremental: 'boolean', protectData: 'boolean', includeParentFields: 'array',
    includeCustomFields: 'array', priorityHeaders: 'array', tabbedStatuses: 'boolean', visibility: 'string'
  }, 'openApplyUpdater_');
  ...
  Logger.log(`Updating ${sheet.sheetName} with id of ${sheet.sheetId} of ss id: ${sheet.id}`);
  ...
  function setupSheet (id, syncKey, since="since_date_${sheet.sheetId}") {
    ...
    if (replies == null) { 
      ...
      if (isIncremental) {
        ...
        Logger.log("Using since_date=" + Utilities.formatDate(since_date, Session.getTimeZone(), 'YYYY-MM-dd hh:mm:ss') + " (local timezone) to request updates since last run");
      }
    }
  ...
  function tabbed () {
    ...
    const {sheet, since_date, md} = setupSheet(id, syncKey, "since_date_${sheet.id}");
    ...
    const jsons = downloadOAv3_2_({endpoint, sheet, since_date, statuses, parentCustomFields: includeParentFields, studentCustomFields: includeCustomFields, protectData, includeIds});
    ...
    for (const status of statuses) {
      ...
      const thisJsons = jsons.filter(j => j.status === status || includeIds.includes(j.id));

      updateSpreadsheet_({
        sheet: thisSheet, jsons: thisJsons, isIncremental,
        priorityHeaders
      });
  ...
  function untabbed() {
    // initialize the gsheet, fetches the sheetId from metadata
    const {sheet, md, since_date} = setupSheet(id, syncKey, "since_date_${sheet.sheetId}");
    const includeIds = grabIds(sheet);
    ...
    updateSpreadsheet_({
      sheet, jsons, isIncremental,
      priorityHeaders
    });

    if (statuses.length > 0)
     pruneWrongStatuses(jsons, sheet, statuses);
